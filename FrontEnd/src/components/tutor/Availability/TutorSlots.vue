<script setup lang="ts">
  ...
  // Convert workingLocation number to array of available location strings
  const availableLocations = computed(() => {
    const locationId = profileStore.workingLocation;
    const locations: string[] = [];

    if (!locationId) return ['online', 'student_home', 'teacher_home']; // Default to all

    // 1 = Teacher's Home only
    // 2 = Online only
    // 3 = Student's Home only
    // 4 = Teacher's Home + Online
    // 5 = Teacher's Home + Student's Home
    // 6 = Online + Student's Home
    // 7 = All three

    if (locationId === 1 || locationId === 4 || locationId === 5 || locationId === 7) {
      locations.push('teacher_home');
    }
    if (locationId === 2 || locationId === 4 || locationId === 6 || locationId === 7) {
      locations.push('online');
    }
    if (locationId === 3 || locationId === 5 || locationId === 6 || locationId === 7) {
      locations.push('student_home');
    }

    return locations.length > 0 ? locations : ['online', 'student_home', 'teacher_home'];
  });
          ...
          try {
            ...
            if (
              ...
              userBookings.forEach((booking: any) => {
                const slotIndex = store.slotsByMonth[currentMonthKey].slotData[
                  currentDay
                ].findIndex((slot) => slot.apiId === booking.slotApiId);

                if (slotIndex >= 0) {
                  store.slotsByMonth[currentMonthKey].slotData[currentDay][slotIndex].studentName =
...
<template>
  <div class="p-3 bg-white shadow-lg sm:p-4 md:p-6 rounded-2xl">
    ...
    <div v-if="loading" class="flex justify-center py-8">
      <div
        class="w-8 h-8 border-4 border-orange-500 rounded-full border-t-transparent animate-spin"
      ></div>
    </div>
    ...
    >
      ...
      <p v-if="!isViewingAsStudent" class="mt-2 text-gray-400">
      ...
      <div class="space-y-2 sm:space-y-3">
        ...
        >
          <div class="flex items-center">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              class="w-3 h-3 mr-1 text-gray-400 sm:w-3.5 sm:h-3.5 sm:mr-1.5"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
          ...
          <div class="flex items-center gap-1 sm:gap-2">
            <span
              :class="[
                'px-4 py-1.5 text-sm font-medium rounded-md whitespace-nowrap',
                slot.activeStatus === true
                  ? isViewingAsStudent && currentUserName === slot.studentName
            ...
            <button
              ...
              :disabled="loading"
              class="px-4 py-1.5 text-sm font-medium text-white bg-purple-600 rounded-md hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
              Book
            </button>
